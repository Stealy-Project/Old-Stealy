...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (args[0] === "stop"){
            ...
            message.edit(client.language("*Rainbow role arrêté", "*Rainbow role stopped"))
        }
        else {
            let role = message.mentions.roles.first() || message.guild.roles.get(args[0])
            if (!role) return message.edit(client.language(`*Aucun rôle de trouvé pour \`${args[0] || "Rien"}\`*`,`*No rôles found for \`${args[0] || "Nothing"}\`*`))
    
            message.edit(client.language("*Rainbow role lancé.*", "*Rainbow Role started.*"))
