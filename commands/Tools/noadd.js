...
module.exports = {
    ...
    */
    run: async (client, message, args) => {
        ...
        switch(args[0]){
            default: return message.edit(client.language('*Argument invalide. Utilisez "on" pour démarrer ou "off" pour arrêter.*', '*Invalid argument. Use "on" to start or "off" to stop.*'))
            ...
            case "on":
                ...
                client.db.antigroup.noadd.push(channel.id);
                client.save();

                message.edit(client.language(`*Personne ne pourra être ajouté au groupe ${channel.name}*`, `*Nobody can be added to the group ${channel.name}*`));
                break
            ...
            case "off":
                ...
                client.db.antigroup.noadd = client.db.antigroup.noadd.filter(c => c !== channel.id)
                client.save();

                message.edit(client.language(`*Les utilisateurs pourront maintenant être ajoutés à ${channel.name}*`, `*Users can bee added to ${channel.name}*`));
                break
        }
    }
};