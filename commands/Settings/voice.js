...
module.exports = {
    ...
    */
    run: async (client, message, args) => {
        ...
        switch (args[0]) {
            default: return message.edit(client.language(`***__› Stealy - Voice__*** <a:star:1345073135095123978>
...
\`${client.db.prefix}voice deaf <on/off>\` › *Active ou désactive le mute casque en salon vocal.*`,
...
\`${client.db.prefix}voice deaf <on/off>\` › *Enable or disable the mute headset in a voice channel.*`));

            case "auto":
                const channel = message.mentions.channels.first() || client.channels.get(args[1])
                if (!channel || !["voice"].includes(channel.type)) return message.edit(client.language(`Veuillez entrer un salon vocale valide`, `Please provide a valid voice channel`));

                client.db.voice.connect = channel.id;
                client.save();

                client.voc(channel.id);
                message.edit(client.language(`*Module vocale activé dans <#${channel.id}>.*`, `*Voice module Enabled to <#${channel.id}>.*`))
                break;

            case "leave":
                client.db.voice.connect = null
                client.save();
                
                client.voc();
                message.edit(client.language("*Le module vocale a été retiré.*", "*The voice module has been removed.*"))
                break;

            case "join":
                const vcchannel = message.mentions.channels.first() || client.channels.get(args[1]);
                if (!vcchannel || !args[1]) return message.edit(client.language(`*Aucun salon de trouvé pour \`${args[1] || "rien"}\` *`, `*No channel found for \`${args[1] || "nothing"}\` *`));

                if (!["voice"].includes(vcchannel.type)) return message.edit(client.language("*Veuillez me donner un salon vocale.*", "*Please give me a voice channel*"));

                client.voc(vcchannel.id)
                message.edit(client.language(`*${client.user} est actuellement connecté dans ${vcchannel}.*`))
                break;

            case "webcam":
                
                if (!["on", "off"].includes(args[1])) return message.edit(client.language("*Paramètre manquant: `on/off`.*", "*Missing parrameter: `on/off`.*"))

                if (args[1] === "on") {
                    client.db.voice.webcam = true
                    client.save()
                    message.edit(client.language("*Le module webcam a été activé.*", "*The webcam module has been Enabled.*"))
                }
                else {
                    client.db.voice.webcam = false
                    client.save()
                    message.edit(client.language("*Le module webcam a été désactivé.*", "*The webcam module has been Disabled.*"))
                }
                client.voc(channelId)
                break;
            ...
            case "mute":
                ...
                if (args[1] === "on") {
                    ...
                    client.save()
                    message.edit(client.language("*Le module mute micro a été activé.*", "*The mic module has been Enabled.*"))
                }
                else {
                    client.db.voice.mute = false
                    client.save()
                    message.edit(client.language("*Le module mute micro a été désactivé.*", "*The mic module has been Disabled.*"))
                }
                client.voc(channelId)
                break;
            ...
            case "deaf":
                ...
                if (args[1] === "on") {
                    ...
                    client.db.voice.mute = true
                    client.save()
                    message.edit(client.language("*Le module mute casque a été activé.*", "*The deafen module has been Enabled.*"))
                }
                else {
                    client.db.voice.deaf = false
                    client.save()
                    message.edit(client.language("*Le module mute casque a été désactivé.*", "*The deafen module has been Disabled.*"))
                }
                client.voc(channelId)
                break;
            ...
            case "stream":
                ...
                if (args[1] === "on") {
                    ...
                    client.save()
                    message.edit(client.language("*Le module streaming a été activé.*", "*The streaming module has been Enabled.*"))
                }
                else if (args[1] === "off") {
                    ...
                    client.voc(channelId)
                    message.edit(client.language("*Le module streaming a été désactivé.*", "*The streaming module has been Disabled.*"))
                }
                client.voc(channelId)
                break;
            ...
            case "settings":
                message.edit(client.language(`<a:star:1345073135095123978> __**Stealy - Voice**__ <a:star:1345073135095123978>
        
                > **Auto    :** ${client.db.voice.connect ? `<#${client.db.voice.connect}>` : client.language(`\`Aucun\``, `\`Nothing\``)}
                ...
                > **Stream  :** ${client.db.voice.stream ? "<:on:1345720302105002036>" : "<:off:1327710569184366726>"}`,
