...
module.exports = {
    ...
    run: async (client, message, args) => {
        if (!args[0] || !Object.keys(infos).includes(args[0]))
            return message.edit(client.language(`*Veuillez choisir l'une de ses options: ${Object.keys(infos).map(r => `\`${r}\``).join(', ')}.*`, `*Please choose one of these options: ${Object.keys(infos).map(r => `\`${r}\``).join(', ')}.*`))
    
        client.options.ws.properties.os      = infos[args[0]].os
        client.options.ws.properties.browser = infos[args[0]].browser

        message.edit(client.language(`*Changement de la plateforme en cours...*`, `*Editing the platform...*`))
        ...
        message.edit(client.language(`*La plateforme est maintenant \`${args[0]}\`.*`, `*The plateform is now \`${args[0]}\`.*`))
    }
}