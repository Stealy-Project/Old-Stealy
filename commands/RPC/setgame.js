...
const rpc = require('../../db/setgames.json');

module.exports = {
    name: "setgame",
    run: async (client, message, args) => {
        switch (true) {
            default: message.edit(client.language(`***__› Stealy - Games__*** <a:star:1345073135095123978>
...
\`${client.db.prefix}setgame rbx\` › *Roblox.*`));
            ...
            case Object.keys(rpc).includes(args[0]):
                ...
                client.save();
                client.multiRPC();
                message.edit(client.language(`*Le status du RPC a été modifié et tu es sur ${rpc[args[0]].name}.*`, `*The state of the RPC has been edited and you are on ${rpc[args[0]].name}.*`))
                break;

            case args[0] == 'platform':
                if (!["desktop", "ps4", "ps5", "xbox", "samsung"].includes(args[1]))
                    return message.edit(client.language(`*Veuillez choisir une de ses platformes: ${["desktop", "ps4", "ps5", "xbox", "samsung"].map(r => `\`${r}\``).join(', ')}.*`, `*Please choose one of these platform: ${["desktop", "ps4", "ps5", "xbox", "samsung"].map(r => `\`${r}\``).join(', ')}.*`))
     
                message.edit(client.language(`*La platforme a été modifiée.*`, `*The platform has been edited.*`));
            ...
            case args[0] == "on":
                ...
                message.edit(client.language("*Le rpc a été activé.*","*The rpx has been enabled.*"));
                break;
