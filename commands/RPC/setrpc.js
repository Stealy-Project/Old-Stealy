...
const rpc = require('../../db/rpcs.json');
    ...
    run: async (client, message, args) => {
        switch(true){
            ...
            case "clear" :
                ...
                client.save();
                client.multiRPC();
                message.edit(client.language("Le RPC a été supprimé", "The RPC has been deleted"));
                break;
            ...
            case Object.keys(rpc).includes(args[0]):
                ...
                message.edit(client.language(`*Le status du RPC a été modifié et tu ${getType(rpc[args[0]].type)[0]} ${args.slice(1).join(' ') || 'Stealy'} sur ${rpc[args[0]].name}.*`, `*The state of the RPC has been edited and you ${getType(rpc[args[0]].type)[1]} ${args.slice(1).join(' ') || 'Stealy'} on ${rpc[args[0]].name}.*`));
                break;
        }
    }
}
