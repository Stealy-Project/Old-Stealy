    ...
    run: async (client, message, args) => {
        ...
        switch (args[0]) {
            case "add" :
                ...
                if (!timeMs) return message.edit(client.language(`*Utilisez un temps valide comme \`20m , 1h , 2d\`.*`,`*Please use a time format such as \`20m , 1h , 2d\`.*`));
            ...
            case "delete" : 
                ...
                if (args[1] < 0 || args[1] > client.db.remind.length) return message.edit(client.language(`*Aucun remind a supprimé.*`, `*No reminder has been deleted.*`))
            ...
            case "list" : 
                if (client.db.remind === 0) return message.edit(client.language(`*Vous n'avez aucun rappel en cours.*`,`*You have no active reminders.*`));
