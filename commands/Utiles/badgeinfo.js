  ...
  run: async (client, message, args) => {
    ...
    client.guilds.forEach(guild => {
      ...
      if (member?.premiumSince) {
        let month = Math.floor((Date.now() - member.premiumSince.getTime()) / (1000 * 60 * 60 * 24 * 30));
        let matchingBadge = Object.keys(emojis).filter(b => b.startsWith("BOOSTER_"))
    ...
    let badgeList = badges.map(b => emojis[b] || null).filter(Boolean).join(" ") || "Aucun badge";
    message.edit(`***Badges de [${user.username}](https://discord.gg/stealy) ***\n${badgeList}`);
  }
};