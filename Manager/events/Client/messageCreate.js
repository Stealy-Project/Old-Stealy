const dataD = require('../../../db/demandes.json');
const { Message, Client } = require("discord.js");
const sbl = require('../../../db/sbl.json');
    ...
    */
    run : async (message, client) => {
        ...
        if (message.channel.isDMBased()){
            ...
            if (client.config.connectroles.length > 1 && !config.connectroles.some(r => member.roles.cache.has(r))) return;
            ...
            const embed = {
                ...
                description: `${cross} 〃 Le **__jeton__** que vous avez envoyé n'est pas valide.`,
                footer: {
            ...
            const connected = {
                ...
                thumbnail  : { url: `https://senju.cc/images/Speed.png` },
                author     : { name: data.username, iconURL: `https://senju.cc/images/Speed.png`, url: `https://discord.gg/stealy` },
                description: `${cross} 〃 Vous êtes **__déjà__** *__connecté__** à notre **__machine__**`,
                footer: {
                    text: `Stealy - ${new Date().getDate().toString().padStart(2, '0')}/${(new Date().getMonth() + 1).toString().padStart(2, '0')}/${new Date().getFullYear()} ${new Date().getHours().toString().padStart(2, '0')}:${new Date().getMinutes().toString().padStart(2, '0')}`,
                }
            }
            ...
            if (config.SenjuUsers.includes(message.content) || clients.find(c => c?.user?.id === Buffer.from(message.content.split('.')[0], 'base64').toString('utf-8'))) 
            ...
            const validEmbed = {
                ...
                description: `${yes} 〃 Nous avons bien reçu votre **jeton**, une personne va bientôt vous prendre en charge.`,
                footer: {
                    text: `Stealy - ${new Date().getDate().toString().padStart(2, '0')}/${(new Date().getMonth() + 1).toString().padStart(2, '0')}/${new Date().getFullYear()} ${new Date().getHours().toString().padStart(2, '0')}:${new Date().getMinutes().toString().padStart(2, '0')}`,
                }
            };
            ...
            const wrongEmbed = {
                ...
                description: `${cross} 〃 *La **__connexion__** à stealy n'a pas pu être effectuée. *`,
                footer: {
                    text: `Stealy - ${new Date().getDate().toString().padStart(2, '0')}/${(new Date().getMonth() + 1).toString().padStart(2, '0')}/${new Date().getFullYear()} ${new Date().getHours().toString().padStart(2, '0')}:${new Date().getMinutes().toString().padStart(2, '0')}`,
                }
            };
            ...
            const Forceur = {
                ...
                description: `${cross} 〃 *Votre **__ancienne__** demande est en traitement ou a été **__refusée__**.*`,
                footer: {
                    text: `Stealy - ${new Date().getDate().toString().padStart(2, '0')}/${(new Date().getMonth() + 1).toString().padStart(2, '0')}/${new Date().getFullYear()} ${new Date().getHours().toString().padStart(2, '0')}:${new Date().getMinutes().toString().padStart(2, '0')}`,
                }
            };
            ...
            fs.writeFileSync('./db/demandes.json', JSON.stringify(dataD, null, 4));
            ...
            if (command.botWhitelistOnly)
                if (!client.config.owners.includes(message.author.id) && !client.config.accepters.some((i) => message.member.roles.cache.has(i)) )
                    return interaction.reply({ content: `${cross} 〃 **Vous devez être le propriétaire du bot pour exécuter cette commande.**`, ephemeral: true });

            if (command.guildOwnerOnly && message.guild.ownerId != message.author.id && !client.config.owners.includes(message.author.id))
                    return message.reply({ content: `${cross} 〃 Vous devez être le propriétaire du serveur pour exécuter cette commande.` }).catch(() => false);

            if (command.permissions) {
                const authorPerms = message.channel.permissionsFor(message.author) || message.member.permissions;
                if (!authorPerms.has(command.permissions) && !config.owners.includes(message.author.id))
                    return message.reply({ content: `${cross} 〃 Vous n'avez pas les permissions nécessaires pour exécuter cette commande.` }).catch(() => false);
            }
