module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (member.roles.cache.has(client.config.wlrole)) return message.channel.send(`${member.user.globalName || member.user.username} est déjà whitelist`)

        member.roles.add(message.guild.roles.cache.get(client.config.wlrole), `Rôle whitelist ajouté par ${message.author.username}`)
    ...
    async executeSlash(client, interaction) {
        ...
        if (member.roles.cache.has(client.config.wlrole)) return interaction.reply({ content: `${user.globalName || user.username} est déjà whitelist`, ephemeral: true })

        member.roles.add(guild.roles.cache.get(client.config.wlrole), `Rôle whitelist ajouté par ${interaction.user.username}`)
            .then( () => interaction.reply({content: `${member.user.globalName || member.user.username} a été **whitelist**`, ephemeral: true }))
            .catch(() => interaction.reply({ content: `Je n'ai pas pu whitelist ${member.user.globalName || member.user.username} `, ephemeral: true }))
    },
    get data() {
        return {
            name: this.name,
