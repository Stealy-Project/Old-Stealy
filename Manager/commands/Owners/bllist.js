...
module.exports = {
    name: "bllist",
    description: "Afficher la liste des utilisateurs blacklist.",
    aliases: [],
    permissions: [],
    ...
    async execute(client, message, args) {
        ...
        let page = 1;
        let count = p1;
        let maxpage = Math.ceil(blacklist.length / count) === 0 ? 1 : Math.ceil(blacklist.length / count)

        const embed = {
            title      : 'Liste des membres blacklist',
            color      : 0xFFFFFF,
            footer     : { text: `Page ${page}/${maxpage}`, iconURL: message.author.avatarURL({ dynamic: true }) },
            description: `${blacklist.length > 0 ? blacklist.map((m,i) => `\`${i+1}\` - <@${m.id}> (\`${m.id}\`)`).slice(p0, p1).join('\n') : "Aucun utilisateur"}`
        }
    ...
    async executeSlash(client, interaction) {
        ...
        collector.on('collect', async i => {
            ...
            if (i.customId === "previous"){
                ...
                embed.footer = ({text: `Page ${page}/${maxpage}`, iconURL: interaction.user.avatarURL})    
                msg.edit({embeds: [embed]})    
            }
